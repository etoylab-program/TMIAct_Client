...
public class UICardEnchantPopup : FComponent
{
    ...
    {        
        ...
        startFX = FuncCreateFX("prf_fx_ui_enchant_start",transform);
        successFX = FuncCreateFX("prf_fx_ui_enchant_success", transform);
        failFX = FuncCreateFX("prf_fx_ui_enchant_fail", transform);
    }
    ...
    {        
        ...
        _CardData = null;
        _CardData = GameInfo.Instance.CardList.Find(x => x.CardUID == uid);
    ...
    public override void Renewal(bool bChildren)
    {
        ...
        kCardTexture.mainTexture = (Texture)ResourceMgr.Instance.LoadFromAssetBundle("card", string.Format("Card/{0}_{1}.png", _CardData.TableData.Icon, GameSupport.GetCardImageNum(_CardData)));

        kGradeSpr.spriteName = string.Format("itemgrade_L_{0}", _CardData.TableData.Grade);
        kWakeSpr.spriteName = string.Format("itemwake_0{0}", _CardData.Wake);
        kWakeSpr.MakePixelPerfect();

        kNameLabel.textlocalize = FLocalizeString.Instance.GetText(_CardData.TableData.Name);        
        kLevelLabel.textlocalize = GameSupport.GetLevelText((int)eTEXTID.LEVEL_TXT_NOW_LV, _CardData.Level, GameSupport.GetMaxLevelCard(_CardData.TableData.Grade, _CardData.Wake));
        ...
        CurLv = _CardData.EnchantLv;
        NextLv = GetNextLevel(CurLv);

        _EnchantTable = GameInfo.Instance.GameTable.Enchants.Find(x => x.GroupID == _CardData.TableData.EnchantGroup && x.Level == CurLv);

        kBeforeLvLabel.textlocalize = string.Format("+{0}", CurLv);
        ...
        kAbilityLabel.textlocalize = string.Format(FLocalizeString.Instance.GetText(1668), GetAbilityValue(_CardData.EnchantLv), GetAbilityValue(_CardData.EnchantLv + 1));

        kNeedGoldLabel.SetActive(true);
        kNeedGoldLabel.textlocalize = "0";
        //재료설정
        IsLackMaterial = false;
        ItemReqList = GameInfo.Instance.GameTable.ItemReqLists.FindAll(x => x.Group == _EnchantTable.ItemReqListID);
        if (ItemReqList != null && ItemReqList.Count > 0)
        {
            ...
            int curSlot = 0;
            for (int i = 0; i < ItemReqList.Count; i++)
            {
                if (curSlot >= kMaterialItemList.Count) break;

                GameTable.ItemReqList.Param p = ItemReqList[i];
                if (FuncSetItemListSlot(curSlot, p.ItemID1, p.Count1)) curSlot++;
                if (FuncSetItemListSlot(curSlot, p.ItemID2, p.Count2)) curSlot++;
                if (FuncSetItemListSlot(curSlot, p.ItemID3, p.Count3)) curSlot++;
                if (FuncSetItemListSlot(curSlot, p.ItemID4, p.Count4)) curSlot++;
            }

            ReqGold = ItemReqList[0].Gold;
            kNeedGoldLabel.textlocalize = string.Format("{0:#,#}", ReqGold);
        }
    }
    ...
    {
        if (_CardData == null || _CardData.TableData == null) return -1;

        var _EnchantTable = GameInfo.Instance.GameTable.Enchants.Find(x => x.GroupID == _CardData.TableData.EnchantGroup && x.Level == curLv + 1);
    ...
    {
        var _EnchantTable = GameInfo.Instance.GameTable.Enchants.Find(x => x.GroupID == _CardData.TableData.EnchantGroup && x.Level == enchantLv);
        if (_EnchantTable == null) return 0;
    ...
    public void OnClick_Enchant()
    {
        ...
        GameInfo.Instance.Send_ReqEnchantCard(_CardData.CardUID, OnNetAckEnchantCard);
        IsResultDirect = true;
    }

    private void OnNetAckEnchantCard(int result, PktMsgType pktmsg)
    {
        if (result != 0)
            return;        

        StartCoroutine(CoDirectStart(pktmsg as PktInfoCardGrow));
    }
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.LeftShift))
        {
            if (!IsResultDirect)
            {
                PktInfoCardGrow pAck = new PktInfoCardGrow();
                pAck.comGrow_ = new PktInfoProductComGrowAck();
                pAck.comGrow_.expLv_ = new PktInfoExpLv();
                pAck.comGrow_.expLv_.lv_ = 1;

                pAck.growState_ = eGrowState.SUCC_NOR;

                pAck.retEnc_ = 10;
                IsResultDirect = true;
                StartCoroutine(CoDirectStart(pAck));
            }
        }
        else if (Input.GetKeyDown(KeyCode.RightShift))
        {
            if (!IsResultDirect)
            {
                ...
                pAck.comGrow_ = new PktInfoProductComGrowAck();
                pAck.comGrow_.expLv_ = new PktInfoExpLv();
                pAck.comGrow_.expLv_.lv_ = 0;

                pAck.growState_ = eGrowState.FAIL;
    ...
    private IEnumerator CoDirectStart(PktInfoCardGrow pAck)
    {
