...
public class ActionBehimosDownhill : ActionEnemyAttackBase
{
    public enum eState
    {
        TakeOff = 0,        // 이륙
    ...
    private Vector3 m_edgePoint = Vector3.zero;
    ...
    public override void Init(int tableId, List<GameTable.CharacterSkillPassive.Param> listAddCharSkillParam)
    {
        ...
        actionCommand = eActionCommand.BehimosDownhill;
        ...
        while (!m_endUpdate)
        {
            switch ((eState)m_state.current)
            {
                ...
                case eState.MoveToEdge:
                    {
                        ...
                        m_owner.cmptMovement.UpdatePosition(v, m_owner.speed * 2.0f, false);
                ...
                case eState.PrepareToAttack:
                    {
                        Vector3 fixedDest = new Vector3(m_dest.x, m_owner.transform.position.y, m_dest.z);
                        m_owner.cmptRotate.UpdateRotation(fixedDest - m_owner.transform.position, false);

                        if (m_owner.cmptRotate.t <= 0.0f)
                        {
                ...
                case eState.Attack:
                    {
                        time += m_owner.fixedDeltaTime / 0.7f;
                        t = Mathf.SmoothStep(0.0f, 1.0f, time);

                        Vector3 v2 = (m_dest - transform.position).normalized;
                        Vector3 v = ((-Vector3.up * (1.0f - t)) + (v2 * t)).normalized;

                        m_owner.cmptMovement.UpdatePosition(v, m_owner.speed * 2.0f, false);

                        Vector3 fixedDest = new Vector3(m_dest.x, m_owner.transform.position.y, m_dest.z);
                        m_owner.cmptRotate.UpdateRotation(m_dest - m_owner.transform.position, false);

                        bool checkRaycastHit = Physics.Raycast(m_owner.transform.position, m_owner.transform.forward, out RaycastHit hitInfo, (m_owner.MainCollider.height * 0.5f) + m_checkDistance, (1 << (int)eLayer.Wall));
                        if (checkRaycastHit || Vector3.Distance(m_owner.transform.position, fixedDest) <= m_checkDistance)
                            m_state.ChangeState(eState.TakeOff, true);
                    }
                    break;
    ...
    private bool ChangeMoveToEdgeState(bool changeAni)
    {
        ...
        {
            m_edgePoint = new Vector3(25.0f, 0.0f, 0.0f);
        ...
        else
        {
            ...
            int index = UnityEngine.Random.Range(0, battleAreaMgr.GetCurrentBattleArea().edgePoints.Length - 1);
            m_edgePoint = battleArea.edgePoints[index].position;
        }
    ...
    private bool ChangePrepareToAttackState(bool changeAni)
    {
        ...
        Vector3 v1 = new Vector3(m_edgePoint.x, m_center.y, m_edgePoint.z);
        float dist = Vector3.Distance(v1, m_center);
        Vector3 dir = (m_center - v1).normalized;

        m_dest = m_center + (dir * dist);
        return true;
    }

    private bool ChangeAttackState(bool changeAni)
    {
        //m_owner.SetSuperArmor(Unit.eSuperArmor.Lv2, GetType());
        m_owner.PlayAniImmediate(eAnimation.FlyMove);
