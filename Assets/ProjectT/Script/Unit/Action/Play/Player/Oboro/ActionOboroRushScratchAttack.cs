...
public class ActionOboroRushScratchAttack : ActionSelectSkillBase
{
    [Header("Property")]
    ...
    public override void Init(int tableId, List<GameTable.CharacterSkillPassive.Param> listAddCharSkillParam)
    {
        ...
        mCurAni = eAnimation.DashAttack2;

        if (mValue1 > 0.0f)
        {
            AniEvent.sEvent atkEvt = m_owner.aniEvent.GetLastAttackEvent(eAnimation.DashAttack2);
            if (atkEvt != null)
            {
                atkEvt.behaviour = eBehaviour.GroggyAttack;
            }
        }
    }
