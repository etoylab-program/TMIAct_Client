...
public class ActionRinChargeAttack : ActionChargeAttack {
	...
	public override void Init( int tableId, List<GameTable.CharacterSkillPassive.Param> listAddCharSkillParam ) {
		...
		mCurAni = eAnimation.AttackCharge;
		if ( mValue1 > 0.0f ) {
	...
	public override void OnStart( IActionBaseParam param ) {
		float addAtkRatio = 0.0f;
		if ( SetAddAction ) {
			addAtkRatio = AddActionValue1;
		}
		...
		for ( int i = 0; i < list.Count; i++ ) {
			list[i].atkRatio = list[i].originalAtkRatio + ( list[i].originalAtkRatio * addAtkRatio );
		}
	}
