...
public class ActionShiranuiChargeSummonAttack : ActionChargeAttack
{
    ...
    public override IEnumerator UpdateAction()
    {
        ...
        else
        {
            ...
            mCurAni = eAnimation.ChargeBrandish;
            if (mValue1 > 0)
            {
                mCurAni = eAnimation.ChargeBrandish2;
            }

			List<AniEvent.sEvent> listAtkEvt = m_owner.aniEvent.GetAllAttackEvent( mCurAni );
			if ( SetAddAction ) {
				for ( int j = 0; j < listAtkEvt.Count; j++ ) {
					listAtkEvt[j].atkRatio += ( listAtkEvt[j].atkRatio * AddActionValue1 );
				}
			}
