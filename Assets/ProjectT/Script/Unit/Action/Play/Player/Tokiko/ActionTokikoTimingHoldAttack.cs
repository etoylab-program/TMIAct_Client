...
public class ActionTokikoTimingHoldAttack : ActionSelectSkillBase
{
    ...
    public override void Init(int tableId, List<GameTable.CharacterSkillPassive.Param> listAddCharSkillParam)
    {
        ...
        if (mValue1 > 0.0f)
        {
            List<Projectile> list = mOwnerPlayer.aniEvent.GetAllProjectile(mCurAni);
            for (int i = 0; i < list.Count; i++)
            {
                list[i].ProjectileAtkAttr = Projectile.EProjectileAtkAttr.STUN;
            }
        }
    }
